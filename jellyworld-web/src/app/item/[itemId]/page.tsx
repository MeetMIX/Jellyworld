...
export default async function ItemPage({ params }: { params: Promise<{ itemId: string }> }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--jw-bg)", color: "var(--jw-text-1)" }}>
      ...
      {/* Infos cheval sur hero */}
      <div style={{
        position: "relative", zIndex: 10, marginTop: "-260px",
        padding: "0 48px", display: "flex", gap: 40, alignItems: "flex-end",
      }}>
        <div style={{
          flexShrink: 0, width: 180, borderRadius: "var(--jw-r-lg)", overflow: "hidden",
          border: "1px solid var(--jw-border)", boxShadow: "0 24px 48px rgba(0,0,0,0.7)",
        }}>
        ...
        <div style={{ flex: 1, paddingBottom: 8, maxWidth: 700 }}>
          {item.Taglines?.[0] && (
            <p style={{ fontSize: 13, fontStyle: "italic", color: "var(--jw-text-3)", margin: "0 0 8px" }}>{item.Taglines[0]}</p>
          )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
            ...
            {item.CriticRating && (
              <span style={{ fontSize: 11, fontWeight: 700, background: item.CriticRating >= 60 ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)", border: `1px solid ${item.CriticRating >= 60 ? "rgba(34,197,94,0.4)" : "rgba(239,68,68,0.4)"}`, color: item.CriticRating >= 60 ? "#4ade80" : "#f87171", borderRadius: 4, padding: "2px 8px" }}>🍅 {item.CriticRating}%</span>
            )}
          ...
          {item.Genres && item.Genres.length > 0 && (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
              {item.Genres.map(g => (
                <span key={g} style={{ fontSize: 11, fontWeight: 600, color: "var(--jw-purple)", background: "rgba(107,47,217,0.12)", border: "1px solid rgba(107,47,217,0.25)", borderRadius: 20, padding: "3px 12px" }}>{g}</span>
              ))}
            </div>
          )}

          {item.Overview && (
            <p style={{ fontSize: 13, color: "var(--jw-text-2)", lineHeight: 1.75, margin: "0 0 16px", maxWidth: 600 }}>{item.Overview}</p>
          )}
          {directors.length > 0 && (
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 28px", borderRadius: "var(--jw-r-md)", background: "var(--jw-gradient)", border: "none", fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.06em" }}>
        ...
        {cast.length > 0 && (
          <section>
            ...
            <div style={{ display: "flex", gap: 16, overflowX: "auto", paddingBottom: 8, scrollbarWidth: "none" }}>
              {cast.map(person => (
                <PersonCard key={person.Id} photoUrl={person.photoUrl} name={person.Name} role={person.Role} />
              ))}
            </div>
          </section>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32 }}>
          <section>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {item.Studios && item.Studios.length > 0 && <div><p style={L}>Studios</p><p style={V}>{item.Studios.map(s => s.Name).join(", ")}</p></div>}
          ...
          <section>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {audioStreams.length > 0 && (
                <div><p style={L}>Audio</p>
                  {audioStreams.map((a, i) => <p key={i} style={{ ...V, margin: "0 0 2px" }}>{a.DisplayTitle}{a.IsDefault ? " (défaut)" : ""}</p>)}
                </div>
              )}
              {subtitleStreams.length > 0 && <div><p style={L}>Sous-titres</p><p style={V}>{subtitleStreams.map(s => s.DisplayTitle || s.Language).filter(Boolean).join(", ")}</p></div>}
